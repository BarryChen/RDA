...
BOOL TM_GetSystemTime(TM_SYSTEMTIME * pSystemTime)
{
    ...
    {
        CSW_TRACE(BASE_TM_TS_ID,"hal_TimRtcGetTime Error\n");
        CSW_PROFILE_FUNCTION_EXIT(TM_GetSystemTime);
        return FALSE;
    }
    ...
    if( TRUE != TM_IsValidTime(pSystemTime) )
    {
      ...
      {
          CSW_TRACE(BASE_TM_TS_ID,"hal_TimRtcSetTime Error\n");
    ...
    if(TRUE != RET)
    {
        CSW_TRACE(BASE_TM_TS_ID,"TM_SystemTimeToFileTime Error \n");
        return FALSE;
    }
    ...
    if(gCSWPowerOnTime && ((nFileTime.DateTime - gCSWPowerOnTime) > 2*60*60))
    {
      ...
      if( ERR_SUCCESS != nRet )
      {
        CSW_TRACE(BASE_TM_TS_ID,"CFW_CfgSetTimeParam Error 0x%x\n",nRet);
        return FALSE;
      }
    ...
    {
      gCSWPowerOnTime = nFileTime.DateTime;
      CSW_TRACE(BASE_TM_TS_ID,"nFileTime - gCSWPowerOnTime %d\n",(nFileTime.DateTime - gCSWPowerOnTime));
    }
    CSW_TRACE(BASE_TM_TS_ID,"TM_GetSystemTime exit\n");
...
)
{
    ...
    {
      SystemTime.uSecond  = RtcTime.sec   = 0;
      SystemTime.uMinute  = RtcTime.min   = 0;
      SystemTime.uHour    = RtcTime.hour  = 10;
      SystemTime.uDay     = RtcTime.day   = 1;
      SystemTime.uMonth   = RtcTime.month = 6;
      SystemTime.uDayOfWeek = RtcTime.wDay  = 5; // It's a Friday.
      RtcTime.year  = 12; //Change default year is 2012.
      SystemTime.uYear = 2012;
      if(hal_TimRtcSetTime(&RtcTime)!= HAL_ERR_NO)
      {
          CSW_PROFILE_FUNCTION_EXIT(TM_GetSystemTime);
          return FALSE;
...
UINT32 TM_PowerOnPatch()
{
  ...
  if( ERR_SUCCESS == bRet )
  {
    CSW_TRACE(BASE_TM_TS_ID,"TM_GetSystemFileTime Success\n");
  }
  else
  {
    CSW_TRACE(BASE_TM_TS_ID,"TM_GetSystemFileTime Error nRet 0x%x\n", nRet );
  }

  nRet = CFW_CfgGetTimeParam(&nSWFileTime);
  if( ERR_SUCCESS == nRet )
  {
    CSW_TRACE(BASE_TM_TS_ID,"CFW_CfgGetTimeParam Success\n");
  }
  else
  {
    CSW_TRACE(BASE_TM_TS_ID,"CFW_CfgGetTimeParam nRet 0x%x\n", nRet );
  }

  if( nSWFileTime.DateTime> nHWFileTime.DateTime )
  {
    CSW_TRACE(BASE_TM_TS_ID,"nSWFileTime > nHWFileTime\n");
    nTimeValidFlag = FALSE;
  }
  else if(( nHWFileTime.DateTime - nSWFileTime.DateTime ) > ( 2*24*60*60 ))
  {
    CSW_TRACE(BASE_TM_TS_ID,"( nHWFileTime - nSWFileTime ) > ( 2*24*60*60 )\n");
    nTimeValidFlag = FALSE;
  }
  else
  {
    CSW_TRACE(BASE_TM_TS_ID,"nTimeValidFlag = TRUE\n");
    nTimeValidFlag = TRUE;
  }
  ...
  if(!nTimeValidFlag)
  {
    ...
    if( ERR_SUCCESS != nRet )
    {
      CSW_TRACE(BASE_TM_TS_ID,"TM_SetSystemTime Error\n");
      return ERR_INVALID_HANDLE;
    }
...
BOOL TM_SetSystemTime(TM_SYSTEMTIME * pSystemTime)
{
    ...
    if( ERR_SUCCESS == nRet )
    {
        CSW_TRACE(BASE_TM_TS_ID,"CFW_CfgSetTimeParam Success\n");
    }
    else
    {
        CSW_TRACE(BASE_TM_TS_ID,"CFW_CfgSetTimeParam nRet 0x%x\n", nRet );
    }
#endif
...
*  BOOL type.TRUE is success,FALSE if fail.
*/
BOOL TIM_ReadFlash(UINT8 *Pdata,UINT32 bytesize)
{
...
//tianwq    if(!CII_Flash_Block_Init(TM_FLASH_DATA,Pdata,bytesize))
#if 0
    {
