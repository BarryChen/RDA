...
{     
     ...
     UINT16 uSize;             // cache buffer size.
...
PRIVATE UINT32 *g_cacheBuff = NULL;
...
INT32 DRV_OpenTFlash(UINT8* dev_name,HANDLE* pdev_handle)
{
    ...
    pTDev->w_cache.uSize = MMC_DEF_SEC_SIZE*4;//sizeof(g_mmc_write_cache);
...
{    
    ...
    { 
        drv_GetCardSize(&all_blk_nr,&sec_sz);  
        if(blk_nr + (tflash_dev->r_cache.uSize/sec_sz) < all_blk_nr)
        {
        ...
        result = mcd_Read(blk_nr, tflash_dev->r_cache.pBuff, size);
		//hal_HstSendEvent(SYS_EVENT,0x99999991);
		//hal_HstSendEvent(SYS_EVENT,result);
        if (result != MCD_ERR_NO )
        {            
            try_times ++;
...
INT32 DRV_WriteTFlash (HANDLE dev_handle, UINT32 blk_nr, CONST BYTE* pBuffer)
{
   ...
   {   
        ...
        do
        {
            ...
            result = mcd_Read(blk_nr, tflash_dev->w_cache.pBuff, size);
            if (result != MCD_ERR_NO)
            {
                try_times ++;
