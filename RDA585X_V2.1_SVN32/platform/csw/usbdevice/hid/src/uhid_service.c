...
PRIVATE UINT32 uHid_MouseDescConstructorAfterInterrupt(UINT8* buffer)
{
	...
	p_desc = sxr_Malloc(sizeof(HAL_USB_HID_DES));
    ...
    sxr_Free(p_desc);
	 ...
	 p_desc = sxr_Malloc(sizeof(HAL_USB_EP_DESCRIPTOR_REAL_T));
...
PUBLIC void uhid_Open(UINT8 *desc, UINT16 length)
{
   ...
   uctls_AddInterface(uctls_NewCSInterfaceDescriptor(uHidDescConstructorAfterInterrupt,0));
