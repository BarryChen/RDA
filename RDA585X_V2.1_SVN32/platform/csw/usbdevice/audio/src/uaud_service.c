...
PRIVATE UINT32 uAudDescConstructorAfterControl(UINT8* buffer)
{
	...
	p_desc = sxr_Malloc(sizeof(AUD_CTL_IF_HEADER_DES));
    ...
    ((AUD_CTL_IF_HEADER_DES*)p_desc)->TotalLength = 0x0029;//0x47;
    ((AUD_CTL_IF_HEADER_DES*)p_desc)->InCollection  = 0x01;//0x02;
...
//    ((AUD_CTL_IF_HEADER_DES*)p_desc)->InterfaceNr2 = 0x02;	
	...
	p_desc = sxr_Malloc(sizeof(AUD_CTL_IN_TRM_DES));
	...
	sxr_Free(p_desc);
#endif	
	p_desc = sxr_Malloc(sizeof(AUD_CTL_OUT_TRM_DES));
	...
	sxr_Free(p_desc);
#endif	
	p_desc = sxr_Malloc(sizeof(AUD_CTL_FTR_UNIT_DES_1));
	...
	sxr_Free(p_desc);
#if 0
	p_desc = sxr_Malloc(sizeof(AUD_CTL_FTR_UNIT_DES_2));
...
PRIVATE UINT32 uAudDescConstructorAfterStreamOut(UINT8* buffer)
{
    ...
    p_desc = sxr_Malloc(sizeof(AUD_STR_IF_DES));
    ...
    p_desc = sxr_Malloc(sizeof(AUD_STR_FMT_TYPE_D_DES));
    ...
    p_desc = sxr_Malloc(sizeof(AUD_STR_ISO_AUD_DATA_EP_DES));
    ...
    ((AUD_STR_ISO_AUD_DATA_EP_DES*)p_desc)->LockDelayUnit = 0x01;
    ((AUD_STR_ISO_AUD_DATA_EP_DES*)p_desc)->LockDelay = 0x0001;
...
//  ((AUD_STR_FMT_TYPE_D_DES*)p_desc)->SamFreq[0][2] = 0x00;
  ...
  ((AUD_STR_FMT_TYPE_D_DES*)p_desc)->SamFreq[0][2] = 0x00;
#if 0
   p_desc = sxr_Malloc(sizeof(AUD_STR_FMT_TYPE_C_DES));
...
{
#if 0
    static UINT32 virtual_audio[45] = {0};
...
PRIVATE HAL_USB_CALLBACK_RETURN_T uaudStreamOutCallbackEp(HAL_USB_CALLBACK_EP_TYPE_T type, HAL_USB_SETUP_T* setup)
{
   ...
   switch(type)
   {
...
#if 0         
 		 memcpy((UINT8 *)MMC_USBGetRXBuffer(), g_UsbAudIsoOutBuf, g_uAudPacketSize);
 		 hal_HstSendEvent(USB_EVENT,0x13030503);
 		 hal_HstSendEvent(USB_EVENT,MMC_USBGetRXBuffer());
 		 hal_HstSendEvent(USB_EVENT,g_UsbAudIsoOutBuf);
		 MMC_USBReceiveData(g_uAudPacketSize);	
		 hal_UsbRecv(g_uaudIsoOutEp,g_UsbAudIsoOutBuf,g_uAudPacketSize,0);
#endif
      ...
      case HAL_USB_CALLBACK_TYPE_CMD:       
         ...
         //hal_HstSendEvent(USB_EVENT,0xb2000002);
...
PUBLIC void uaud_Open(void)
{
 ...
 //  g_UsbAudIsoOutBuf = (UINT8*)sxr_Malloc(USB_AUD_ISO_OUT_BUF_SIZE);
 //  g_UsbAudCmdDataBuf = (UINT8*)sxr_Malloc(USB_AUD_CMD_DATA_BUF_SIZE);
   ...
   uctls_AddInterface(uctls_NewCSInterfaceDescriptor(uAudDescConstructorAfterControl,0));	
#if 0
...
//   epNum = uctls_AllocateEndPoint();	
   ...
   uctls_AddInterface(uctls_NewCSInterfaceDescriptor(uAudDescConstructorAfterStreamOut,0));		
