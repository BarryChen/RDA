                       ...
                       UINT8*                   usbSerialNumber)
{
    ...
    if(g_UctlsStatus == UCTLS_STATUS_CLOSE)
    {
        ...
        sxr_Malloc(sizeof(HAL_USB_INTERFACE_DESCRIPTOR_T*) * 16); // max 16 interface
        hal_HstSendEvent(USB_EVENT,0x09040001);
		hal_HstSendEvent(USB_EVENT,usbConfigs[0]->interfaceList);
...
PUBLIC VOID uctls_SetMode(UCTLS_ID_T mode)
{
    ...
    hal_HstSendEvent(SYS_EVENT,modes);
    ...
    for(i = 0, nbInterface = 0; i < UCTLS_SERVICE_LIST_SIZE; ++i)
    {
        ...
        hal_HstSendEvent(SYS_EVENT,modes[g_UctlsId].servicesList[i].serviceCallback);
    }
    ...
    if(nbInterface != 0)
    {
		...
		hal_HstSendEvent(SYS_EVENT,interfaceList);
