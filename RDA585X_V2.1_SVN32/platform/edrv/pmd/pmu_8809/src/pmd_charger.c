...
PRIVATE UINT16 pmd_GetEffuseValue(PMD_EFFUSE_PAGE_T page)
{
    ...
    value = ((value| ((((value&0x3c00)>>10) == 0) ? 0 : 1<<(((value&0x3c00)>>10)-1)) )&0x3ff) ;

    return value;
}

PRIVATE UINT16 pmd_GetRawEffuseValue(PMD_EFFUSE_PAGE_T page)
{
    UINT16 value;
    value = pmd_OpalSpiRead(RDA_ADDR_EFUSE_OPT_SETTING1);
    value = PMU_SET_BITFIELD(value, RDA_PMU_EFUSE_SEL_WORD_REG, page);
...
PRIVATE UINT16 pmd_GetEffuseValue2(PMD_EFFUSE_PAGE_T page)
{
    UINT16 value;
    value = pmd_OpalSpiRead(RDA_ADDR_EFUSE_OPT_SETTING1);
    value = PMU_SET_BITFIELD(value, RDA_PMU_EFUSE_SEL_WORD_REG, page);
...
{
	UINT32 val =((pmd_GetEffuseValue2(PMD_EFFUSE_PAGE_CHARGER)>>10)&0xf)<<10;	
	val +=((pmd_GetEffuseValue2(PMD_EFFUSE_PAGE_VBAT_3P4)>>10)&0x1f)<<5;
	val +=((pmd_GetEffuseValue2(PMD_EFFUSE_PAGE_VBAT_4P2)>>10)&0x1f);
