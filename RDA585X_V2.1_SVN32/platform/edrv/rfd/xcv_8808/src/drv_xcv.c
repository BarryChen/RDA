...
#undef SPI_REG_DEBUG
#undef SWITCH_AUDIO_CODEC_FREQ
#undef FM_LDO_WORKAROUND
#endif

//#ifdef FASTPF_USE_USB
// XCV chip ID is needed for 8808 USB control.
// Undefine internal function/data attribute to allow discarding
// useless function and data sections. Otherwise the lod is too large.
...
CONST XCV_RX_GAIN_T AgcTab_EGSM_8808[AGC_QTY] = EGSM_AGC_8808;
CONST XCV_RX_GAIN_T AgcTab_DCS_8808[AGC_QTY] = DCS_AGC_8808;
CONST XCV_RX_GAIN_T AgcTab_EGSM_8808S[AGC_QTY] = EGSM_AGC_8808S;
CONST XCV_RX_GAIN_T AgcTab_DCS_8808S[AGC_QTY] = DCS_AGC_8808S;

CONST XCV_RX_GAIN_T* AgcTab_EGSM = NULL;
CONST XCV_RX_GAIN_T* AgcTab_DCS = NULL;
...
volatile UINT16 RFD_DATA_INTERNAL g_xcvCalibPaRampValue[4];

#ifdef SWITCH_AUDIO_CODEC_FREQ
VOLATILE UINT32 RFD_DATA_INTERNAL g_xcvAudFreqVal_09h;
VOLATILE UINT32 RFD_DATA_INTERNAL g_xcvAudFreqVal_0ah;
VOLATILE UINT32 g_xcvAudFreqStatus = 0;
...
#ifdef SWITCH_AUDIO_CODEC_FREQ
    if (g_xcvAudFreqStatus == 1)
    {
        // Add audio piggyback commands
        // switch to the page 4
        xcvBuildSpiCmd (spiCmd, 0x3f, 0x00004);
        xcvBuildSpiCmd (spiCmd, 0x09, g_xcvAudFreqVal_09h);
        xcvBuildSpiCmd (spiCmd, 0x0a, g_xcvAudFreqVal_0ah);
        // back to page 0
        xcvBuildSpiCmd (spiCmd, 0x3f, 0x00000);
        g_xcvAudFreqStatus = 2; // Audio piggyback commands have been saved in queue
    }
#endif
...
#ifdef SWITCH_AUDIO_CODEC_FREQ
VOID rfd_XcvSwitchAudioCodecFreq(UINT32 freqVal_09h, UINT32 freqVal_0ah)
{
    #define MAX_PIGGYBACK_WAIT_COUNT 3
    UINT32 count;
    ...
    g_xcvAudFreqVal_09h = freqVal_09h;
    g_xcvAudFreqVal_0ah = freqVal_0ah;
    ...
    {
        // Piggyback audio commands with XCV Rx commonds
        g_xcvAudFreqStatus = 1;
        count = 0;
        while (count++ < MAX_PIGGYBACK_WAIT_COUNT)
        {
            ...
            if (g_xcvAudFreqStatus == 4)
            {
                // Audio piggyback commands have been sent to XCV
                hal_SysExitCriticalSection(status);
                return;
            }
        }
        // Cancel audio piggyback commands
        g_xcvAudFreqStatus = 0;
    }

    // Now we can directly write to XCV
    // switch to the page 4
    rfd_XcvRegForceWriteSingle(0x3f, 0x00004);
    rfd_XcvRegForceWriteSingle(0x09, freqVal_09h);
    rfd_XcvRegForceWriteSingle(0x0a, freqVal_0ah);
    // back to page 0
...
UINT8 RFD_FUNC_INTERNAL rfd_XcvRxOn(RFD_WIN_T *Win, INT16 start)
{
    ...
    spiCmd = hal_RfspiNewCmd(  (start + XCV_TIME(SPI_CMD) - CMD_DURATION(15)),
                                Win->winIdx );
...
VOID rfd_XcvReadChipId(VOID)
{
    if (g_xcvChipId == 0)
    {
...
#ifdef FPGA
        g_xcvChipId = XCV_CHIP_ID_8808_V2;
#else
        if (hal_SysGetChipId(HAL_SYS_CHIP_PROD_ID) == HAL_SYS_CHIP_PROD_ID_8808)
        {
            UINT32 count = 0;
            while (!(g_xcvChipId >= XCV_CHIP_ID_8808_V2
                     && g_xcvChipId <= XCV_CHIP_ID_8808_VF))
            {
                if (count++ == 100)
                {
                    hal_HstSendEvent(SYS_EVENT,XCV_CHIP_ID_INVALID_EVENT);
                    hal_HstSendEvent(SYS_EVENT,g_xcvChipId);
                }
        ...
        {
            // 8808S
            g_xcvChipId = XCV_CHIP_ID_8808_V4;
        }
#endif
        hal_RfspiClose();
    }
...
INLINE VOID xcvConfigure(VOID)
{
    ...
    // ;=====Reg Reset==============================================
    if (g_xcvChipId != XCV_CHIP_ID_8808_V2)
    {
    ...
    // Direct-reg pd_xtal=0 before enabling 6p5m to avoid 26M clock burr
    xcvBuildSpiCmd (confCmd, 0x20, 0x2108d);
    xcvBuildSpiCmd (confCmd, 0x3F, 0x00003); // page=3
    xcvBuildSpiCmd (confCmd, 0x01, 0x3e410); // Enable_clk_6p5m=1
    // Restore pd_xtal setting after enabling 6p5m
    xcvBuildSpiCmd (confCmd, 0x3F, 0x00000); // page=0
    xcvBuildSpiCmd (confCmd, 0x20, 0x0108d);
    xcvBuildSpiCmd (confCmd, 0x3F, 0x00003); // page=3
    hal_RfspiSendCmd(confCmd);
    xcvBuildSpiCmd (confCmd, 0x04, 0x043f0); // clk32k_lpo_en_pmu=0
    hal_RfspiSendCmd(confCmd);
    hal_SysWaitMicrosecond(100); //Delay 100us
    xcvBuildSpiCmd (confCmd, 0x04, 0x203f0); // clk32k_26mXtal_en_pmu=1
    hal_RfspiSendCmd(confCmd);
#endif
    //xcvBuildSpiCmd (confCmd, 0x3F, 0x00000); // page 0
    ...
    xcvBuildSpiCmd (confCmd, 0x00, 0x100| ((lp_CDAC&RDA6220_CDAC_REG_MASK) << RDA6220_CDAC_REG_OFFSET));
    xcvBuildSpiCmd (confCmd, 0x3f, 0x00000); // page 0
    hal_RfspiSendCmd(confCmd);

    // ;=====Page 4=====
    xcvBuildSpiCmd (confCmd, 0x3f, 0x00004); // page 4
    xcvBuildSpiCmd (confCmd, 0x01, 0x04f26); // pll_cpaux_bit=7
    xcvBuildSpiCmd (confCmd, 0x03, 0x04f26); // pll_aux_cpaux_bit=7
    xcvBuildSpiCmd (confCmd, 0x0b, 0x00001); // divider 1/4
    if (hal_SysGetChipId(HAL_SYS_CHIP_PROD_ID) == HAL_SYS_CHIP_PROD_ID_8808)
    {
        xcvBuildSpiCmd (confCmd, 0x0d, 0x09041); // dcdc divider
        xcvBuildSpiCmd (confCmd, 0x0e, 0x09041); // dcdc divider
        xcvBuildSpiCmd (confCmd, 0x0f, 0x08015); // divider 1/6
    }
    else // 8808S
    {
        xcvBuildSpiCmd (confCmd, 0x0d, 0x093fd); // dcdc divider
        xcvBuildSpiCmd (confCmd, 0x0e, 0x093fd); // dcdc divider
        xcvBuildSpiCmd (confCmd, 0x0f, 0x0805d); // divider 1/24
    }
    // ;=====DCXO Setting=====
    UINT16 adjustedCdac = g_CDAC + g_CdacDelta;
    xcvBuildSpiCmd (confCmd, 0x3f, 0x00000); // page 0
    ...
    #else//warkey 2.1
    xcvBuildSpiCmd (confCmd, RDA6220_CDAC_REG, 0x5C11);
    #endif
    hal_RfspiSendCmd(confCmd);

#ifdef SWITCH_AUDIO_CODEC_FREQ
    xcvBuildSpiCmd (confCmd, 0x3f, 0x00004); // page 4
    xcvBuildSpiCmd (confCmd, 0x03, 0x04922); // int_mode
    xcvBuildSpiCmd (confCmd, 0x04, 0x0c202); // sdm_en
    if (g_xcvChipId == XCV_CHIP_ID_8808_V2)
    {
        xcvBuildSpiCmd (confCmd, 0x09, 0x0c000); // pll_sdm2_freq
        xcvBuildSpiCmd (confCmd, 0x0a, 0x0000f); // pll_sdm2_freq
    }
    else
    {
        xcvBuildSpiCmd (confCmd, 0x09, 0x0B57B); // pll_sdm2_freq
        xcvBuildSpiCmd (confCmd, 0x0a, 0x0FB9F); // pll_sdm2_freq
    }
    xcvBuildSpiCmd (confCmd, 0x0b, 0x00001); // divider 1/4
    if (hal_SysGetChipId(HAL_SYS_CHIP_PROD_ID) == HAL_SYS_CHIP_PROD_ID_8808)
    {
    ...
    xcvBuildSpiCmd (confCmd, 0x0b, 0x00001); // divider 1/4
    xcvBuildSpiCmd (confCmd, 0x0f, 0x08015); // divider 1/6
    xcvBuildSpiCmd (confCmd, 0x0a, 0x0fb9f); //
    xcvBuildSpiCmd (confCmd, 0x09, 0x0b57b); //
    xcvBuildSpiCmd (confCmd, 0x3f, 0x00000); // page 0
    hal_RfspiSendCmd(confCmd);
#endif
#if 0
    // ;=====Page 4=====Test audio 52 28
    xcvBuildSpiCmd (confCmd, 0x3f, 0x00004); // page 4
    xcvBuildSpiCmd (confCmd, 0x04, 0x0c202); // sdm_en
    xcvBuildSpiCmd (confCmd, 0x03, 0x04922); // int_mode
    xcvBuildSpiCmd (confCmd, 0x0b, 0x00001); // divider 1/4
    xcvBuildSpiCmd (confCmd, 0x0f, 0x08031); // divider 1/13
    xcvBuildSpiCmd (confCmd, 0x0a, 0x0000f); // *28
    xcvBuildSpiCmd (confCmd, 0x09, 0x0e000); //
    xcvBuildSpiCmd (confCmd, 0x3f, 0x00000); // page 0
    hal_RfspiSendCmd(confCmd);
#endif
}
...
VOID rfd_XcvOpen(INT32 AfcOffset, CONST XCV_CONFIG_T* xcvConfig)
{
    ...
    if (hal_SysGetChipId(HAL_SYS_CHIP_PROD_ID) == HAL_SYS_CHIP_PROD_ID_8808)
    {
        AgcTab_EGSM = AgcTab_EGSM_8808;
        AgcTab_DCS = AgcTab_DCS_8808;
    }
    else // 8808S
    {
        AgcTab_EGSM = AgcTab_EGSM_8808S;
        AgcTab_DCS = AgcTab_DCS_8808S;
    }
...
VOID rfd_XcvRfInit(VOID)
{
    ...
    // ;=====IC Debug=====
    xcvBuildSpiCmd (confCmd, 0x09, 0x3c299); // all Ldo bypass1=1
    xcvBuildSpiCmd (confCmd, 0x07, 0x38300); // for pa ramp
    xcvBuildSpiCmd (confCmd, 0x10, 0x088ff); // 
    xcvBuildSpiCmd (confCmd, 0x11, 0x0024a); // xpll_fbc_sel=010
    xcvBuildSpiCmd (confCmd, 0x14, 0x261b4); // 
    if (hal_SysGetChipId(HAL_SYS_CHIP_PROD_ID) == HAL_SYS_CHIP_PROD_ID_8808)
    {
        xcvBuildSpiCmd (confCmd, 0x17, 0x013a8); // dac_rst_time=11
    }
    else // 8808S
    {
        xcvBuildSpiCmd (confCmd, 0x17, 0x013bf); // dac_rst_time=11;Rmx_lo_div_ibit_900=1111
        xcvBuildSpiCmd (confCmd, 0x0e, 0x38440); // Rmx_lo_div_ibit_1800=1100
    }
    xcvBuildSpiCmd (confCmd, 0x18, 0x0354c); // adc_reset_delay=14CH(filter reset)
    xcvBuildSpiCmd (confCmd, 0x1a, 0x2f57c); // xtal_hlsel=1111(digAFC range),Xtal_reg_bit=100(3.2M rx block tone)
    xcvBuildSpiCmd (confCmd, 0x1b, 0x2f951); // thmt_dac_ref
    xcvBuildSpiCmd (confCmd, 0x1c, 0x12080); // Thmt_ptat_var;Thermo_cal_cnt
    xcvBuildSpiCmd (confCmd, 0x20, 0x0108d); // 
    xcvBuildSpiCmd (confCmd, 0x21, 0x09999); // all ldo bypass2=1

#if 0
    // ;=====Tx Setting=====
    xcvBuildSpiCmd (confCmd, 0x02, 0x2ab55); // Digrf_stream_mode_enable=1
    xcvBuildSpiCmd (confCmd, 0x03, 0x00320); // Ramp_curve,Ramp_mult_factor
    xcvBuildSpiCmd (confCmd, 0x28, 0x24602); // equfil_tx_bypass=1,TX_diff_en=1
    hal_RfspiSendCmd(confCmd);
    
    // Tx timing control
	xcvBuildSpiCmd (confCmd, 0x24, 0x20740); // 17:16 txpower start delay 2
    xcvBuildSpiCmd (confCmd, 0x3e, 0x35df5); // Tx timer control
    ...
    // ;=====RX Setting=====
    xcvBuildSpiCmd (confCmd, 0x01, 0x3c036); // rx gain set
    if (hal_SysGetChipId(HAL_SYS_CHIP_PROD_ID) == HAL_SYS_CHIP_PROD_ID_8808)
    {
        xcvBuildSpiCmd (confCmd, 0x0d, 0x287e4); // for RX ADC 
    }
    else // 8808S
    {
        xcvBuildSpiCmd (confCmd, 0x0d, 0x09775); // for RX ADC 
    }
    xcvBuildSpiCmd (confCmd, 0x29, 0x08c00); // Bw_sel=001
    xcvBuildSpiCmd (confCmd, 0x3b, 0x00685); // mode=slow,time=10(80us),bw=101(60Hz)
    xcvBuildSpiCmd (confCmd, 0x3f, 0x00001); // page 1
    xcvBuildSpiCmd (confCmd, 0x30, 0x0cc84); // 17:16 txpower start delay 2
    xcvBuildSpiCmd (confCmd, 0x31, 0x00fed); // 17:16 txpower start delay 2   xcvBuildSpiCmd (confCmd, 0x44, 0x00289); // 
    xcvBuildSpiCmd (confCmd, 0x44, 0x00289); // 
    if (hal_SysGetChipId(HAL_SYS_CHIP_PROD_ID) == HAL_SYS_CHIP_PROD_ID_8808)
    {
        xcvBuildSpiCmd (confCmd, 0x4c, 0x07ffe); // gain table7
        xcvBuildSpiCmd (confCmd, 0x4b, 0x07fca); // gain table6
        xcvBuildSpiCmd (confCmd, 0x4a, 0x001ca); // gain table5
        xcvBuildSpiCmd (confCmd, 0x49, 0x000ca); // gain table4
        xcvBuildSpiCmd (confCmd, 0x48, 0x000c2); // gain table3
        xcvBuildSpiCmd (confCmd, 0x47, 0x00002); // gain table2
        xcvBuildSpiCmd (confCmd, 0x46, 0x00000); // gain table1
        xcvBuildSpiCmd (confCmd, 0x45, 0x00000); // gain table0
    }
    else // 8808S
    {
        xcvBuildSpiCmd (confCmd, 0x4c, 0x07ffe); // gain table7
        xcvBuildSpiCmd (confCmd, 0x4b, 0x049fe); // gain table6
        xcvBuildSpiCmd (confCmd, 0x4a, 0x049de); // gain table5
        xcvBuildSpiCmd (confCmd, 0x49, 0x000de); // gain table4
        xcvBuildSpiCmd (confCmd, 0x48, 0x0004e); // gain table3
        xcvBuildSpiCmd (confCmd, 0x47, 0x00006); // gain table2
        xcvBuildSpiCmd (confCmd, 0x46, 0x00001); // gain table1
        xcvBuildSpiCmd (confCmd, 0x45, 0x00000); // gain table0
    }
    hal_RfspiSendCmd(confCmd);

    xcvBuildSpiCmd (confCmd, 0x60, 0x1e927); // Lna_1800_i=111
//    xcvBuildSpiCmd (confCmd, 0x70, 0x3ffff); // Thermo_diff(0111~0000,1111~1000)
//    xcvBuildSpiCmd (confCmd, 0x71, 0x3ffff); // Thermo_diff
    xcvBuildSpiCmd (confCmd, 0x77, 0x004f8); // Thermo_diff_en
#endif
    xcvBuildSpiCmd (confCmd, 0x3f, 0x00000); // page 0

    // ;=====PLL Setting=====
    xcvBuildSpiCmd (confCmd, 0x31, 0x02aac); // Equfil_tx_g_gsm_d8
    xcvBuildSpiCmd (confCmd, 0x3f, 0x00001); // page 1
    xcvBuildSpiCmd (confCmd, 0x67, 0x030c5); // Equfil_tx_g_gsm_d7
    xcvBuildSpiCmd (confCmd, 0x68, 0x025ee); // Equfil_tx_g_gsm_d9
    xcvBuildSpiCmd (confCmd, 0x69, 0x02223); // Equfil_tx_g_gsm_d10
    xcvBuildSpiCmd (confCmd, 0x41, 0x002ab); // PLL_850
    xcvBuildSpiCmd (confCmd, 0x42, 0x0029b); // PLL_900
    xcvBuildSpiCmd (confCmd, 0x43, 0x0029b); // PLL_1800
    xcvBuildSpiCmd (confCmd, 0x44, 0x0028b); // PLL_1900
    //xcvBuildSpiCmd (confCmd, 0x3f, 0x00000); // page 0
    hal_RfspiSendCmd(confCmd);

    // ;=====IC Self-Cal=====
    //xcvBuildSpiCmd (confCmd, 0x3f, 0x00001); // page 1
    xcvBuildSpiCmd (confCmd, 0x63, 0x20235); // Dc_cal_ mode=00
    xcvBuildSpiCmd (confCmd, 0x6a, 0x38073); // polarity
    xcvBuildSpiCmd (confCmd, 0x3f, 0x00000); // page 0
#ifdef CHANNEL_FREQUENCY_MODE
    xcvBuildSpiCmd (confCmd, 0x30, 0x00557); // DigRF=0,Freq_mode=1,Mdll_dr=1,Self-Cal=1
#else
    xcvBuildSpiCmd (confCmd, 0x30, 0x00517); // DigRF=0,Freq_mode=0,Mdll_dr=1,Self-Cal=1
#endif
    xcvBuildSpiCmd (confCmd, 0x39, 0x28000); // GPIO_BS_RX; Close switch when calib
    xcvBuildSpiCmd (confCmd, 0x08, 0x140af); // Mdll_div=01,band_sel =00,arfcn=175
    xcvBuildSpiCmd (confCmd, 0x0a, 0x18001); // pdn=1

    hal_RfspiSendCmd(confCmd);
    sxr_Sleep(200 MS_WAITING);

    xcvBuildSpiCmd (confCmd, 0x0a, 0x18000); // pdn=0
    xcvBuildSpiCmd (confCmd, 0x08, 0x15025); // Mdll_div=01,band_sel =01,arfcn=37
    xcvBuildSpiCmd (confCmd, 0x0a, 0x18001); // pdn=1
    ...
    xcvBuildSpiCmd (confCmd, 0x0a, 0x18000); // pdn=0
    xcvBuildSpiCmd (confCmd, 0x08, 0x1629d); // Mdll_div=01,band_sel =10,arfcn=669
    xcvBuildSpiCmd (confCmd, 0x0a, 0x18001); // pdn=1
    ...
    xcvBuildSpiCmd (confCmd, 0x0a, 0x18000); // pdn=0
    xcvBuildSpiCmd (confCmd, 0x08, 0x17276); // Mdll_div=01,band_sel =11,arfcn=630
    xcvBuildSpiCmd (confCmd, 0x0a, 0x18001); // pdn=1
...
#ifdef CHANNEL_FREQUENCY_MODE
    xcvBuildSpiCmd (confCmd, 0x30, 0x00755); // DigRF=1,Freq_mode=1,Mdll_dr=1,Self-Cal=0
#else
    xcvBuildSpiCmd (confCmd, 0x30, 0x00715); // DigRF=1,Freq_mode=0,Mdll_dr=1,Self-Cal=0
#endif
	// ;=====Mode Setting=====
