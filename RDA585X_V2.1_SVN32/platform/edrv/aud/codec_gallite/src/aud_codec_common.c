...
////////////////////////////////////////////////////////////////////////////////
//                                                                            //
//  $HeadURL: http://svn.rdamicro.com/svn/developing1/Workspaces/aint/bv5_dev/platform/edrv/aud/codec_gallite/src/aud_codec_common.c $ //
//	$Author: admin $                                                        // 
...
PRIVATE VOID aud_CodecCommonCodecPrePowerOnConfig(VOID)
{
    ...
    index = aud_CodecCommonGetRateIndex(g_audCodecSampleRate);

#if (CHIP_ASIC_ID != CHIP_ASIC_ID_GALLITE && \
     CHIP_ASIC_ID != CHIP_ASIC_ID_8808) // 8809 or later
    hal_AbbRegWrite(CODEC_SDM2_FREQ_HIGH, g_audCodecFreqRegVals[index].sdm2FreqHigh);
    hal_AbbRegWrite(CODEC_SDM2_FREQ_LOW, g_audCodecFreqRegVals[index].sdm2FreqLow);

#ifdef ABB_HP_DETECT
    // Generate 20 Hz signal
    hal_AbbRegWrite(CODEC_HP_DETECT_3EH, g_audHpDetectMp3RegVals[index].hpDet_3eh);
    hal_AbbRegWrite(CODEC_HP_DETECT_3FH, g_audHpDetectMp3RegVals[index].hpDet_3fh);
#endif // ABB_HP_DETECT

#ifdef AUD_3_IN_1_SPK
    // Set notch filter coefficients
    // Filter out 170 Hz signal
    hal_AbbRegWrite(CODEC_HP_DETECT_44H, g_aud3In1SpkMp3RegVals[index].hpDet_44h);
    hal_AbbRegWrite(CODEC_HP_DETECT_45H, g_aud3In1SpkMp3RegVals[index].hpDet_45h);
    hal_AbbRegWrite(CODEC_HP_DETECT_46H, g_aud3In1SpkMp3RegVals[index].hpDet_46h);
    hal_AbbRegWrite(CODEC_HP_DETECT_47H, g_aud3In1SpkMp3RegVals[index].hpDet_47h);
#else // !AUD_3_IN_1_SPK
#ifdef ABB_HP_DETECT
    // Set notch filter coefficients
    // Filter out 20 Hz signal
    hal_AbbRegWrite(CODEC_HP_DETECT_44H, g_audHpDetectMp3RegVals[index].hpDet_44h);
    hal_AbbRegWrite(CODEC_HP_DETECT_45H, g_audHpDetectMp3RegVals[index].hpDet_45h);
    hal_AbbRegWrite(CODEC_HP_DETECT_46H, g_audHpDetectMp3RegVals[index].hpDet_46h);
    hal_AbbRegWrite(CODEC_HP_DETECT_47H, g_audHpDetectMp3RegVals[index].hpDet_47h);
#endif // ABB_HP_DETECT
#endif // !AUD_3_IN_1_SPK
#endif // 8809 or later
}
     ...
     CHIP_ASIC_ID != CHIP_ASIC_ID_8808) // 8809 or later
// =============================================================================
// aud_CodecCommonFmPrePowerOnConfig
// -----------------------------------------------------------------------------
/// Set registers before powering on FM.
/// @return
// =============================================================================
PRIVATE VOID aud_CodecCommonFmPrePowerOnConfig(VOID)
{
    // Set sample rate, in fact to change reg_2c_osr for fm record
    aud_CodecCommonSetRateStg(g_audCodecSampleRate);

#ifdef ABB_HP_DETECT
    // Generate 20 Hz signal
    hal_AbbRegWrite(CODEC_HP_DETECT_3EH, g_audHpDetectFmRegVals.hpDet_3eh);
    hal_AbbRegWrite(CODEC_HP_DETECT_3FH, g_audHpDetectFmRegVals.hpDet_3fh);
#endif
#ifdef AUD_3_IN_1_SPK
    // Set notch filter coefficients
    // Filter out 170 Hz signal
    hal_AbbRegWrite(CODEC_HP_DETECT_44H, g_audHpDetectFmRegVals.hpDet_44h);
    hal_AbbRegWrite(CODEC_HP_DETECT_45H, g_audHpDetectFmRegVals.hpDet_45h);
    hal_AbbRegWrite(CODEC_HP_DETECT_46H, g_audHpDetectFmRegVals.hpDet_46h);
    hal_AbbRegWrite(CODEC_HP_DETECT_47H, g_audHpDetectFmRegVals.hpDet_47h);
#endif
}
#endif // 8809 or later
                                        ...
                                        BOOL mono)
{
...
#if (CHIP_ASIC_ID != CHIP_ASIC_ID_GALLITE && \
     CHIP_ASIC_ID != CHIP_ASIC_ID_8808) // 8809 or later
        callback = &aud_CodecCommonFmPrePowerOnConfig;
#endif
    }
    ...
    else
    {
     ...
     hal_AudSetSampleRateDiv(sampleRate);
        ...
        BOOL ret ;
        hal_HstSendEvent(SYS_EVENT, 0xc0dec555);
        hal_HstSendEvent(SYS_EVENT, g_audHalCurrentUser);
        hal_HstSendEvent(SYS_EVENT, g_audCodecCurrentUser);
        ret = hal_AudOpen(g_audCodecCurrentUser, callback);
...
PUBLIC AUD_ERR_T aud_CodecCommonSetup(SND_ITF_T itf, CONST AUD_DEVICE_CFG_T* cfg)
{
    ...
    INT8 regAnaLvl, regDacLvl;
    CALIB_BUFFER_T* calibPtr = tgt_GetCalibConfig();
    
    AUD_ASSERT(calibPtr->aud.gainItfMap[itf]>=0, "AUD: No Gain Calibration for itf: %d", itf);
    CALIB_AUDIO_OUT_GAINS_T *pCalibOutGains = 
        &calibPtr->aud.audioGains[calibPtr->aud.gainItfMap[itf]].outGains[cfg->level->spkLevel];
...
#else
/*        if(g_audCodecLatestCfg.spkSel == AUD_SPK_I2S)
        {
            hal_HstSendEvent(SYS_EVENT, 0x10250203);
            pmd_EnablePower(PMD_POWER_LOUD_SPEAKER, FALSE);
            hwp_sysCtrl->Cfg_Reserved &= ~0x02;
        }
*/
        ...
        case AUD_SPK_LOUD_SPEAKER_EAR_PIECE:
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    pEpGains = &calibPtr->aud.audioGains[calibPtr->aud.gainItfMap[SND_ITF_EAR_PIECE]].outGains[cfg->level->spkLevel];
                    if (g_audCodecVoiceQuality)
                    {
          ...
          cfg->micSel == AUD_MIC_LOUD_SPEAKER )
    {
        ...
        if (cfg->level->micLevel != SND_MIC_MUTE)
        {
            ...
            inGains = calibPtr->aud.audioGains[calibPtr->aud.gainItfMap[itf]].inGains;
            anaVol = inGains.ana;
            adcVol = inGains.adc;
