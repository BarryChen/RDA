...
#define TSD_DEBOUNCE_TIME (5*HAL_TICK1S/1000)
#define TSD_REPITITION_TIME (5*(TSD_DEBOUNCE_TIME))


// TSD key stuff
#define TSD_KEY_NUMBER 20
#define TSD_KEY_FILTER_COUNT 2

#define TSD_SAMPLES_TIMES 1
...
PRIVATE INT32 tsd_GetKeyIndex(HAL_ANA_GPADC_MV_T volt)
{
    ...
    app_trace(7,  "tsd_GetKeyIndex volt=%d,index=%d,value=%d", volt, index,(int)(value*100.0));
...
PRIVATE UINT32 tsd_GetScreenPoints(TSD_POINT_T* screenPoint)
{
   ...
   // lcdScreenTouchPoint.y = 0;
	...
	if(global_temp_count<TSD_SAMPLES_TIMES)
	{
	...
	else
	{
    	if(global_temp_count==TSD_SAMPLES_TIMES)
    	{
    	    global_result_all=global_result_all/TSD_SAMPLES_TIMES;
    	}
