...
////////////////////////////////////////////////////////////////////////////////
//                                                                            //
//  $HeadURL: http://10.10.100.14/svn/developing1/Workspaces/aint/bv5_dev/platform/edrv/lcdd/st7735r/gouda/src/drv_lcd_st7735r.c $ //
//	$Author: pengzg $                                                        // 
//	$Date: 2012-03-08 10:28:04 +0800 (æ˜ŸæœŸå›? 08 ä¸‰æœˆ 2012) $                     //   
//	$Revision: 13954 $                                                          //   
//                                                                            //
////////////////////////////////////////////////////////////////////////////////
//                                                                            //
...
PRIVATE VOID lcddp_Init(VOID)
{
...
#ifdef LCD_ST7735S
        hal_HstSendEvent(SYS_EVENT, 0x58300001);
        LCM_WR_CMD(0xB1);
        LCM_WR_DAT(0x05);
        LCM_WR_DAT(0x3C);
        LCM_WR_DAT(0x3C);
        ...
        LCM_WR_DAT(0x04);
        LCM_WR_DAT(0x13);
        hal_HstSendEvent(SYS_EVENT, 0x58300002);
#elif !defined(lcd_line_8_low)
		//------------------------------------ST7735R Frame Rate-----------------------------------------//
		LCM_WR_CMD(0xB1);
...
void dsp_single_colour(int DH, int DL)
{
 int i,j;
 hal_HstSendEvent(SYS_EVENT, 0x58300003);
 //RamAdressSet();
 for (i=0;i<160;i++)
    for (j=0;j<128;j++)
        {
...
PRIVATE BOOL lcddp_CheckProductId()
{
	...
	hal_HstSendEvent(SYS_EVENT, 0x55558888);
	hal_HstSendEvent(SYS_EVENT, productId);
...
PUBLIC LCDD_ERR_T lcdd_update_screen(UINT8* Data, LCDD_ROI_T *rect)
{
    LCDD_FBW_T fb;
    fb.fb.buffer = Data;
    fb.fb.colorFormat = LCDD_COLOR_FORMAT_RGB_565;
    fb.fb.width = 160;
    fb.fb.height= 128;
    fb.roi = *rect;
    lcddp_Blit16(&fb, rect->x, rect->y);
}
