...
PROTECTED void start_flash_operation(UINT8 cmd, UINT32 addr, UINT8 opmode, UINT16 blocksize)
{
   ...
   if((g_spiflash_ID.manufacturerID == MEMD_FLASH_MICRON) && (cmd == g_dif_opcode_high_p_mode) ) return;
...
PUBLIC void write_flash_status(UINT32 flash_addr, UINT8 status_val)
{
    ...
    if(FLASH_TYPE_ID==0)
    {
	   if((g_spiflash_ID.manufacturerID == MEMD_FLASH_MXIC)||
          (g_spiflash_ID.manufacturerID == MEMD_FLASH_ESMT && g_spiflash_ID.device_memory_type_ID == MEMD_FLASH_ESMT_18V)|| 
          (g_spiflash_ID.manufacturerID == MEMD_FLASH_PM25L))
    	{
    		opmode = 0;
        	data_array[0] = status_val;
        	data_array[1] = 0x0;
       ...
       if(g_spiflash_ID.manufacturerID == MEMD_FLASH_ESMT && g_spiflash_ID.device_memory_type_ID == MEMD_FLASH_ESMT_30V)
       {
            opmode = 0;
        	data_array[0] = status_val;
        	data_array[1] = 0x0;
...
PUBLIC BOOL spi_flash_quard_ctrl(BOOL quard_en_flag, UINT8 dual_mode)
{
    ...
    {
		if((g_spiflash_ID.manufacturerID == MEMD_FLASH_MXIC) ||
           (g_spiflash_ID.manufacturerID == MEMD_FLASH_ESMT) ||
           (g_spiflash_ID.manufacturerID == MEMD_FLASH_PM25L))
				status_val = 0x40;
			else  
			if(g_spiflash_ID.manufacturerID == MEMD_FLASH_MICRON)
			{
			     // 0x68  6 dummy clcyle -> 78 M or 104M
			     status_val = 0x68; //mon_Event(0xeeee4321); // 4 line 0x68;   1  line 0x88
			}
			else
				status_val = 0x02;
    }
    ...
    if(g_spiflash_ID.manufacturerID == MEMD_FLASH_MICRON)
    {
        status_val = 0x88; //mon_Event(0xeeee1234); // 4 line 0x68;   1  line 0x88
    }
    else
        status_val = 0x0;
	...
	if(g_spiflash_ID.manufacturerID == MEMD_FLASH_MICRON)
    {
        write_flash_status_micron(0,status_val);
    }
	else		
...
PUBLIC BOOL spi_flash_block_erase_bak(UINT32 flash_addr)
{
    UINT32 data_tmp_32;
    UINT32 addr_tmp;
    UINT8 data_array[3];
...
//    UINT32 addr_tmp;
	...
	#ifdef dual_spi_flash
    spi_flash_ini(TRUE,0x1,clk_div,0x1);
	#else
    spi_flash_ini(TRUE,0x1,clk_div,0x0);
	#endif  
...
*/ 
#if 0
	mon_Event(0x55555666);
	mon_Event(g_spiflash_ID.manufacturerID);
	mon_Event(g_spiflash_ID.device_memory_type_ID);
	mon_Event(g_spiflash_ID.device_capacity_type_ID);
#endif

    if(g_spiflash_ID.manufacturerID == MEMD_FLASH_EON)
    { 
    	mon_Event(0xaaaabbbb);
#if (CHIP_ASIC_ID == CHIP_ASIC_ID_GALLITE)   
	#ifdef dual_spi_flash
        spi_flash_ini(TRUE,0x2,clk_div,0x1);
	#else
		...
		spi_flash_ini(TRUE,0x2,clk_div,0x1);
	#else
        if(g_spiflash_ID.device_memory_type_ID == MEMD_FLASH_EON_18V)
		    spi_flash_ini(TRUE,0x1,clk_div,0x0);
        else
		spi_flash_ini(TRUE,0x2,clk_div,0x0);
	#endif
    ...
    else if(g_spiflash_ID.manufacturerID == MEMD_FLASH_PM25L)
	{
		g_spiflash_pagesize = 256;
		g_dif_opcode_high_p_mode = 0xa3; //NOP
    ...
    else  if(g_spiflash_ID.manufacturerID == MEMD_FLASH_EON)
    {
		g_spiflash_pagesize = 256;
...
PUBLIC UINT32 memd_FlashErase2SuspendTime(VOID)
{
	...
	if(g_spiflash_ID.manufacturerID == MEMD_FLASH_GIGADEVICE)
