...
{
	ASSERT(((UINT32)resource->stack & 3)==0);
	ASSERT(((UINT32)resource->tree & 3)==0);
	ASSERT(((UINT32)resource->GCT & 3)==0);
	ASSERT(((UINT32)resource->LCT & 3)==0);
...
kal_bool GIF_CheckGuardPatternValid(void)
{
	...
	stack = (UINT32*)(gif_resource.stack+gif_resource.stack_size-4);
	tree = (UINT32*)(gif_resource.tree+gif_resource.tree_size-4);
...
void GIF_GetInfo(gif_info_struct *out)
{
	...
	out->height = (UINT16)(reg & 0xFFFF); 
	out->width = (UINT16)(reg >> 16);
	
	reg = DRV_Reg32(GIF_IMG_REG_2);
	out->y = (UINT16)(reg & 0xFFFF); 
	out->x = (UINT16)(reg >> 16);
	...
	out->lheight = (UINT16)(reg & 0xFFFF); 
	out->lwidth = (UINT16)(reg >> 16);
...
*
*************************************************************************/
INT32 GIF_DecodeFirst(UINT8* src, void* dest, UINT32 size)
{
	UINT32 m,n;
...
*
*************************************************************************/
INT32  GIF_DecodeNext(void* dest)
{
	UINT8 status;
	UINT8 bg_index;
	UINT16 *gct_ptr;
   ...
   GPTI_StartItem(gif_dcb.gpt_handle,
	...
	gct_ptr = (UINT16*)gif_resource.GCT;
...
//	g2d.dest_base_address = (UINT32)s->shadow_adrs+(s->shadow_width*g2d.dest_y+g2d.dest_x)*2;
	g2d.dest_base_address = (UINT32)s->shadow_adrs;
...
//	g2d.dest_pitch = s->shadow_width*2;
//	g2d.src_base_address = (UINT32)s->work_adrs+((g2d.src_y*info->width)+g2d.src_x)*2;	
//	g2d.dest_base_address = (UINT32)s->shadow_adrs+((g2d.dest_y*s->shadow_width)+g2d.dest_x)*2;	
	g2d.dest_pitch_width = s->shadow_width;
	g2d.src_base_address = (UINT32)s->work_adrs;	
	g2d.dest_base_address = (UINT32)s->shadow_adrs;	
...
*
*************************************************************************/
INT32 GIF_Display_HW(gif_para_struct *s, gif_info_struct *info)
{
	UINT8 *GIF_src, *GIF_end;
	UINT32 n; 
	...
	ptr = (UINT32*)(gif_resource.stack+gif_resource.stack_size-4);
	*ptr = GIF_GUARD_PATTERN;
	ptr = (UINT32*)(gif_resource.tree+gif_resource.tree_size-4);
	*ptr = GIF_GUARD_PATTERN;	
   ...
   do
   {
      ...
      {	
			...
			case ',': // 0x2C, image descriptor							
						w = (UINT16)(GIF_src[5]<< 8)|(UINT16)GIF_src[4];
						h = (UINT16)(GIF_src[7]<< 8)|(UINT16)GIF_src[6];
