...
HANDLE recHandle = -1;
INT32 *sndRecBuffer = NULL;//[SNDRECBUF_LEN];// the whole record buffer
INT8 *sndRecRingBuffer = NULL;//[SNDRECBUF_LEN/2]; //half of the whole buffer for each half(consider the end as the second half) has a inturrupt

INT32 sndRecBufferLength = 0;// the whole record buffer[SNDRECBUF_LEN];// the whole record buffer
ARS_REC_MODE_T mmc_amrcodec= 0xff;
mci_type_enum mmcRecFormat = -1;
UINT32 g_audioRecSampleRate = 0;
UINT32 g_audioRecCount = 0;
UINT8 g_audioRecNB = 0;
...
INT32 sndRec_writeWaveData(HANDLE fhd, UINT8 *pcm_data, UINT length)
{
    INT32 result=-1;
    int i;
    int len;

    /*if(g_audioRecNB == HAL_AIF_MONO)
    {
        for(i=1;i<length;i+=4)
        {      
            sndRecRingBuffer[i/4] = pcm_data[i]+0x80;
        }
        len = length/4;
    }
    else*/
    {
        ...
        {      
            sndRecRingBuffer[i/2] = pcm_data[i]+0x80;
        }
        len = length/2;
    }

    result=FS_Write(fhd, sndRecRingBuffer, len);
...
INT32 sndRec_writeWaveHeader(HANDLE fhd)
{
	...
	waveFormatHeader.nChannels = g_audioRecNB;//HAL_AIF_MONO;
	waveFormatHeader.nSamplesPerSec = g_audioRecSampleRate;//8000;
	waveFormatHeader.wBitsPerSample = 8;//16;
...
AUD_ITF_T   rec_audioItf = 0;
BOOL rec_paused = FALSE;
//common-
INT32 Mmc_MicStartRecord(UINT32 *pBuffer, UINT32 nBufferSize,ARS_REC_MODE_T codec,BOOL loop_mode,ARS_USER_HANDLER_T recHanlder)
{
...
#ifdef SOUND_RECORDER_SUPPORT_16K	
    g_audioRecSampleRate = HAL_AIF_FREQ_16000HZ;// HAL_AIF_FREQ_16000HZ;//HAL_AIF_FREQ_8000HZ;
#else
    g_audioRecSampleRate = HAL_AIF_FREQ_8000HZ;
#endif
    g_audioRecNB = HAL_AIF_MONO;
    ...
    stream.mode = codec;
    stream.sampleRate = g_audioRecSampleRate;//HAL_AIF_FREQ_8000HZ;
    stream.channelNb = g_audioRecNB;//HAL_AIF_MONO;
#ifdef SOUND_RECORDER_SUPPORT_16K
...
INT32 Mmc_FmStartRecord(UINT32 *pBuffer, UINT32 nBufferSize,ARS_REC_MODE_T codec,BOOL loop_mode,ARS_USER_HANDLER_T recHanlder)
{
    ...
    g_audioRecBufLen = nBufferSize*4;
    g_audioRecBufOffset = 0;
    g_audioRecSampleRate = FMR_Get_SampleRate();//HAL_AIF_FREQ_22050HZ; // HAL_AIF_FREQ_8000HZ;// HAL_AIF_FREQ_16000HZ;//HAL_AIF_FREQ_8000HZ;
    g_audioRecNB = HAL_AIF_STEREO;
    ...
    arsAudioCfg.micLevel = AUD_MIC_ENABLE;
#else
...
//	UINT8 waveHeader[WAVE_FILE_HEADER_SIZE]={0};
	...
	sndRecBuffer=pcmbuf_overlay; //(INT32 *)mmc_MemMalloc(SNDRECBUF_LEN*4);
	sndRecRingBuffer=inputbuf_overlay;//(INT32 *)mmc_MemMalloc(SNDRECBUF_LEN*2);
...
INT32 Mmc_sndRecStop(void)
{
    ...
    if(BT_IsActive())
    {
        DM_StopAudio(); // the bt record will stop with sco stop in simulous.
    }
    else
#endif
    {
...
{
    return ((UINT64)g_audioRecCount) * ((UINT64)g_audioRecBufLen) / g_audioRecNB / (g_audioRecSampleRate/500);
}
     ...
     if(g_RecordPlayOpen==1)
     	{
		    ...
		    if(part == SNDREC_DATA_PART_HALF)
		    {
		            ...
		            case MCI_TYPE_WAV_DVI_ADPCM:
				{
            		if(g_SndRecUserHandler != NULL)
            		{
            		    g_SndRecUserHandler((UINT8 *)sndRecBuffer, sndRecBufferLength*2, (unsigned int *)&result);
            		}
            		else
                        result = sndRec_writeWaveData(recHandle, sndRecBuffer, sndRecBufferLength*2);
		            }
                    hal_HstSendEvent(SYS_EVENT, 0x10702530);
		                break;
		            default:
		                break;
		        }
		...
		if(g_SndRecUserHandler != NULL)
		{
		    g_SndRecUserHandler(((UINT16*)sndRecBuffer)+sndRecBufferLength, sndRecBufferLength*2, (unsigned int *)&result);
		}
		else
                        result = sndRec_writeWaveData(recHandle, ((UINT16*)sndRecBuffer)+sndRecBufferLength, sndRecBufferLength*2);
		            }
                    hal_HstSendEvent(SYS_EVENT, 0x10702560);
		            default:
		                break;
		        }
		    }
