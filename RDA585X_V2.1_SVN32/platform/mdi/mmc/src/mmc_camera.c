...
//    ev.nParam2 = CAPTURE_IMAGE_8M;//for delete nParam2
...
void Cam_CallbackFun(UINT8 id,  UINT8 ActiveBufNum)
{
    ...
    if (id == 1) hal_HstSendEvent(SYS_EVENT,0xe0);
...
void Camera_UserMsg(COS_EVENT *pnMsg)
{
    ...
    switch (pnMsg->nEventId)
    {
        ...
        case MSG_CAM_FULL_DATA:
        {
            ...
            {
                hal_HstSendEvent(SYS_EVENT,0xffff0020);
                hal_HstSendEvent(SYS_EVENT,hal_TimGetUpTime());
                blendEnable = TRUE;
            }
