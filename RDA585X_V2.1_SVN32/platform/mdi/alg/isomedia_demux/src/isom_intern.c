...
int gf_isom_open_file(HANDLE file_handle, uint8 file_mode, uint32 data_Len, GF_ISOFile **movie)
{
	...
	if(!mov)
	{
		diag_printf("[VID] WARNING,isom_intern.c,line:%d, gf_isom_open_file() return -1\n",__LINE__);
		return -1;
	}
	bs_glob=NULL;
	...
	*movie=mov;
	...
	if (e) 
	{
		diag_printf("[VID]isom_intern.c,line:%d, after gf_isom_datamap_new(), e=%d\n",__LINE__,e);
		return -1;
	}
	...
	*/
	...
	{
		diag_printf("[VID] WARNING,isom_intern.c,line:%d, gf_isom_open_file()!INCOMPLETE_FILE,e=%d\n",__LINE__,e);
		return 0;
	}
	else if (e)
	{			
		diag_printf("[VID] WARNING,isom_intern.c,line:%d, gf_isom_open_file()!e=%d\n",__LINE__,e);
		return -1;
	}
...
//#define OUT_A
	...
	fileIn=fopen("F:\\测试文件\\3gp\\mode7_122.3gp","rb");
	//fileIn=fopen("H:\\yea.mp4","rb");
	fileOut=fopen("F:\\测试文件\\avstream.3gp","wb");
	buf=malloc(100*1024);
