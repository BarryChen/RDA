...
#define SXS_TRACE_BUFF_SIZE             (512)

PUBLIC VOLATILE UINT16 HAL_UNCACHED_BSS g_TraceBitMap [SXS_NB_ID];
...
void sxs_vprintf (u32 Id, const ascii *Fmt, va_list argPtr)
{
    ...
    if(g_halHostConfig.trace_enable == 0)
        return;
    if(g_TraceBitMap[TGET_ID(Id)] == 0)
        return;
    if((g_TraceBitMap[TGET_ID(Id)]&(1L<<TGET_LEVEL(Id)))==0 && (Id & TSTDOUT)==0)
        return;
#if 1
    memset(sxs_trace_buf, 0, sizeof(sxs_trace_buf));
    out_buf = vsnprintf(sxs_trace_buf, sizeof(sxs_trace_buf), Fmt, argPtr);
    ...
    out_buf = vsnprintf(sxs_trace_buf+trace_pos+2, SXS_TRACE_BUFF_SIZE/2-4, Fmt, argPtr);
    ...
    if(trace_pos)
        ...
        trace_pos = SXS_TRACE_BUFF_SIZE/8;
#endif
}
