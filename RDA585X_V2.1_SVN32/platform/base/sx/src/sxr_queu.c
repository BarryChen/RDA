...
--------------------------------------------------------------------------------
  ...
  History    :
--------------------------------------------------------------------------------
  Jul 26 03  |  ADA  | Creation
  Oct 01 04  |  ADA  | sxr_QueueGetNext Check that current point a valid position.
  Jan 10 05  |  JFN  | Set Name field to NIL in free queue + consistency check.
             |  ADA  | Load field set to SXR_NO_ENV when queue is not created.
             |       | Added Parameter Print to New function. It allows the user
             |       | to print the items the way he wants in the Check function.
             |       | Field Name and Print handled only when the flag
             |       | __SXR_QUEUE_DBG__ is set (default).
  Mar 16 06  |       | - sxr_NewQueue: call CheckAllQueues in case no more queue
             |       | is available.
  Mar 22 06  |  ADA  | - sxr_CheckQueue: Print / queue not Env...
================================================================================
*/
...
PUBLIC BOOL _sxr_QueueLast (void *Data, u8 Queue, BOOL highPrio)
{
    ...
    if (LIKELY(available))
    {
        ...
        if (sxr_QueueDesc [Queue].Last != SXR_NO_ENV)
         sxr_QueueEnv [sxr_QueueDesc [Queue].Last].Next = QueueEnv -> Idx;
        else
         sxr_QueueDesc [Queue].First = QueueEnv -> Idx;

        sxr_QueueDesc [Queue].Prev [0] = sxr_QueueDesc [Queue].Last;
        sxr_QueueDesc [Queue].Last = QueueEnv -> Idx;
        sxr_QueueDesc [Queue].Cur [0] = QueueEnv -> Idx;
...
PUBLIC BOOL _sxr_QueueNext (void *Data, u8 Queue, BOOL highPrio)
{
    ...
    if (sxr_QueueDesc [Queue].Cur [0] == SXR_NO_ENV)
    {
        if (sxr_QueueDesc [Queue].First == SXR_NO_ENV)
        {
            return _sxr_QueueFirst (Data, Queue, highPrio);
        }
        else
        {
    ...
    {
        QueueEnv -> Data = Data;
        QueueEnv -> Next = sxr_QueueEnv [sxr_QueueDesc [Queue].Cur [0]].Next;
        ...
        sxr_QueueEnv [sxr_QueueDesc [Queue].Cur [0]].Next = QueueEnv -> Idx;
        sxr_QueueDesc [Queue].Prev [0] = sxr_QueueDesc [Queue].Cur [0];
        sxr_QueueDesc [Queue].Cur [0] = QueueEnv -> Idx;
...
void *sxr_QueueGetNext (u8 Loop, u8 User, u8 Queue)
{
 ...
 if (sxr_QueueDesc [Queue].Cur [User] == SXR_NO_ENV)
// &&  (sxr_QueueDesc [Queue].First != SXR_NO_ENV))
 {
  if (Loop)
   sxr_QueueDesc [Queue].Cur [User] = sxr_QueueDesc [Queue].First;
//  else
//   SXS_RAISE ((_SXR|TABORT|TDB,TSTR("No current item in queue.\n",0x06a80007)));
 }

// if (sxr_QueueDesc [Queue].First != SXR_NO_ENV)
 if (sxr_QueueDesc [Queue].Cur [User] != SXR_NO_ENV)
 {
  sxr_QueueDesc [Queue].Prev [User] = sxr_QueueDesc [Queue].Cur [User];
  sxr_QueueDesc [Queue].Cur [User] = sxr_QueueEnv [sxr_QueueDesc [Queue].Cur [User]].Next;
  ...
  if (sxr_QueueDesc [Queue].Cur [User] != SXR_NO_ENV)
   Data = sxr_QueueEnv [sxr_QueueDesc [Queue].Cur [User]].Data;
 }
