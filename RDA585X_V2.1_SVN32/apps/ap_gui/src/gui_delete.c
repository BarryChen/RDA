...
BOOL del_audio_files(TCHAR *Ext)
{
	...
	while(FS_GetNextEntry(&inode, Ext, FS_DIR_FILE) == ERR_SUCCESS)//循环删除几种扩展名的文件
	{
	...
	while(1)
	{
		...
		case AP_KEY_MODE | AP_KEY_UP:
			if(delflag == 1)                      //YES时按下MODE键的处理
			{
				...
				GUI_ResShowMultiString(GUI_STR_CDELING, string_id, 16, 0);     //显示“删除中...”
				GUI_UpdateScreen(NULL);
				if(type == DELETE_FILE)
				{
				...
				else if(type == DELETE_MUSIC)
				{
					...
					{
						GUI_ResShowMultiString(GUI_STR_CDELFAIL, string_id, 16, 0);     //删除失败界面显示
						GUI_UpdateScreen(NULL);
						MESSAGE_Sleep(4);
						return RESULT_IGNORE;
					}
...
DELPLAYLISTDIR:
					while (1)
					{
						...
						{
							return RESULT_IGNORE;    //删文件及修改playlist
						}
					}
					return RESULT_IGNORE;
				}
