...
{
	INT32 i;
	UINT8 *data_addr, *res_addr = gpResourceBase;
...
Menu_t *gui_get_menu(UINT32 menu_id, MenuItem_t **items)
{
	...
	while(nLast >= nFirst)
	{
		...
		{
			*items = gpMenuDataBase + gpMenuList[nMid].offset;
			return gpMenuList + nMid;
		}
...
UINT32 gui_convert_unicode_data(UINT32 nCh, UINT8 font, UINT8 *buff)
{
	...
	pRangeData = pBaseData + gpCurrentFont->pUnicodeRanges;
